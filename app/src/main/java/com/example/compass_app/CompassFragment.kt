    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (lastMagnetometerSet && lastAccelerometerSet) {
            ...
            if (SensorManager.getRotationMatrix(rm, null, lastAccelerometer, lastMagnetometer)) {
                ...
                geoField = GeomagneticField(currentLoc.latitude.toFloat(), currentLoc.longitude.toFloat(), currentLoc.altitude.toFloat(), System.currentTimeMillis())
                azimuth += geoField.declination
                var bearing = currentLoc.bearingTo(destinationLoc)
                var direction = azimuth - bearing
                mRotate(-direction)
                */
